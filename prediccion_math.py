...
def build_modelo(X_train, y_train, pCV, pJobs):
   model_math = eNetCv(alphas=np.linspace(0.001, 10, 100), cv = pCV, n_jobs=pJobs, verbose=0)
...
def implemento_modelo(elCSV, pTrainTest, pCV, pJobs):
  ...
  # reemplazamos los espacios por _ 
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=pTrainTest)

    modelo = build_modelo(X_train, y_train, pCV, pJobs)
